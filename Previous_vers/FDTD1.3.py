...
class Source:
    ...
    def guassian(self,time_step): 
        return np.exp(-((self.t0-time_step) / self.spread) ** 2)
...
def main():
    source = Source()
    FDTD = Grid(shape = (201,None), cell_spacing=1e-3)
    FDTD.set_source(source.guassian, position = [100], active_time = 1000)
    #FDTD.add_dieletric(pos = (100,200), eps=1.7, conductivity=0.04)
    FDTD.run(1000)
